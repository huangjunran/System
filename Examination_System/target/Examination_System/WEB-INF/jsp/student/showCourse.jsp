				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${courseList}" var="item">
								<tr>
									...
									<%--<button class="btn btn-default btn-xs btn-info" onClick="location.href='/student/stuSelect?id=${item.courseid}'">选择</button>--%>
									<c:choose>
										<c:when test="${item.studentid==0}">
											<td>
												<button class="btn btn-default btn-xs btn-info" onClick="stuSelect(${item.courseid})">选择</button>
											</td>
										</c:when>
										<c:otherwise>
											<td>选题已被确定</td>
			...
			$.ajax({
				...
				success : function (data) {  // readystate == 4 && status == 200
					...
					if(data.data!=""){
						...
						window.location.href=('../../student/selectCourse');
					}
				},
				error:function(arg1){
					alert("error"+arg1)
