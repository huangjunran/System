						...
						<div class="row">
							...
							<form class="bs-example bs-example-form col-md-5" role="form" style="margin: 20px 0 10px 0;" action="/admin/selectCourse" id="form1" method="post">
								<div class="input-group">
									<input type="text" class="form-control" placeholder="请输入课题明名" name="findByName">
							...
							<button class="btn btn-default col-md-2" style="margin-top: 20px" onClick="location.href='/admin/coursePass'">
								一键审核
								<sapn class="glyphicon glyphicon-plus"/>
							</button>
				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${courseList}" var="item">
								<tr>
									...
									<td>
                                        <button class="btn btn-default btn-xs btn-info" onClick="location.href='/admin/passCourse?id=${item.courseid}'">
                                            <c:if test="${item.pass==1}">未通过</c:if>
                                            <c:if test="${item.pass==0}">通过</c:if>
                                        </button>
										<%--<button class="btn btn-default btn-xs btn-info" onClick="location.href='/admin/passCourse?id=${item.courseid}'">通 过</button>--%>
